...
- **Vercel** — import the repository and deploy, no configuration needed. Dynamic
  search API, GitHub releases proxy, the works.
- **GitHub Pages** — fully static: `yarn build:static` produces a static export
  (static Orama search index included), and the bundled workflow
  `.github/workflows/deploy-pages.yml` publishes it automatically on every push to
  `main`. See the [Search guide](/docs/guides/search) for details.
